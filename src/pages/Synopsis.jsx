...
const SynopsisForm = React.memo(({ synopsis, onSubmit, onCancel, sharedDocuments, onRemoveDocument }) => {
  const [formData, setFormData] = useState(synopsis || {
    tender_name: "",
    customer_name: "",
    submission_date: "",
    prebid_meeting: "",
  ...
  const analyzeRFPDocument = useCallback(async (documentUrl, documentName) => {
    ...
    try {
      const prompt = `As an expert RFP analyzer for ESDS, extract the following 14 fields from the attached document.
...
14. dr`;
      ...
      const response = await InvokeLLM({
        ...
        response_json_schema: {
          ...
          properties: {
            ...
            prebid_meeting: { type: "string" },
            prebid_query_submission_date: { type: "string" },
            consultant_name: { type: "string" },
            consultant_email: { type: "string" },
            help_desk: { type: "string" },
            tender_fee: { type: "string" },
            tender_emd: { type: "string" },
            branches: { type: "string" },
            cbs_software: { type: "string" },
            dc: { type: "string" },
            dr: { type: "string" }
          },
          required: [
            "tender_name", "customer_name", "submission_date", "prebid_meeting", 
            "prebid_query_submission_date", "consultant_name", "consultant_email", 
            "help_desk", "tender_fee", "tender_emd", "branches", "cbs_software", "dc", "dr"
          ]
        }
      });
  ...
  const handleDocumentSelect = useCallback(async (docUrl) => {
  ...
  const handleReanalyze = useCallback(() => {
    ...
    const docName = formData.rfp_document_name || 
      sharedDocuments.find(doc => doc.url === selectedDocument)?.name;
    ...
    try {
      ...
      if (uploadResult?.file_url) {
        const documentData = {
          ...
          type: file.type,
          size: file.size,
          uploaded_from: "synopsis"
        };

        // Here we assume UploadedDocument.create can return the created doc or just confirm success
        // In a real app, you might want to add this to the sharedDocuments state to make it selectable immediately
        await UploadedDocument.create(documentData); // This line ensures the document is saved.
        setSelectedDocument(uploadResult.file_url);
        
        // Only set document info, do NOT auto-analyze
        setFormData(prev => ({
          ...prev,
          rfp_document_url: uploadResult.file_url,
      ...
      const htmlContent = `
        ...
        <body>
          ...
          <div class="section">
            ...
            <div class="field"><span class="field-label">Tender Name:</span> <span class="field-value">${formData.tender_name || 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Customer Name:</span> <span class="field-value">${formData.customer_name || 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Submission Date:</span> <span class="field-value">${formData.submission_date ? format(new Date(formData.submission_date), 'dd/MM/yyyy') : 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Pre-bid Meeting:</span> <span class="field-value">${formData.prebid_meeting ? format(new Date(formData.prebid_meeting), 'dd/MM/yyyy HH:mm') : 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Pre-bid Query Date:</span> <span class="field-value">${formData.prebid_query_submission_date ? format(new Date(formData.prebid_query_submission_date), 'dd/MM/yyyy') : 'Not specified'}</span></div>
          </div>

          <div class="section">
            <div class="section-title">CONTACT INFORMATION</div>
            <div class="field"><span class="field-label">Consultant Name:</span> <span class="field-value">${formData.consultant_name || 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Email ID:</span> <span class="field-value">${formData.consultant_email || 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Help Desk:</span> <span class="field-value">${formData.help_desk || 'Not specified'}</span></div>
          </div>

          <div class="section">
            <div class="section-title">FINANCIAL DETAILS</div>
            <div class="field"><span class="field-label">Tender Fee:</span> <span class="field-value">${formData.tender_fee ? `₹${Number(formData.tender_fee).toLocaleString()}` : 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Tender EMD:</span> <span class="field-value">${formData.tender_emd ? `₹${Number(formData.tender_emd).toLocaleString()}` : 'Not specified'}</span></div>
            <div class="field"><span class="field-label">Branches:</span> <span class="field-value">${formData.branches || 'Not specified'}</span></div>
          </div>

          <div class="section">
            <div class="section-title">TECHNICAL REQUIREMENTS</div>
            <div class="field"><span class="field-label">CBS Software:</span> <span class="field-value">${formData.cbs_software || 'Not specified'}</span></div>
            <div class="field"><span class="field-label">DC (Data Center):</span> <span class="field-value">${formData.dc || 'Not specified'}</span></div>
            <div class="field"><span class="field-label">DR (Disaster Recovery):</span> <span class="field-value">${formData.dr || 'Not specified'}</span></div>
          </div>
...
<div class="field"><span class="field-label">Submission Date:</span> ${formData.submission_date ? format(new Date(formData.submission_date), 'dd/MM/yyyy') : 'Not specified'}</div>
<div class="field"><span class="field-label">Pre-bid Meeting:</span> ${formData.prebid_meeting ? format(new Date(formData.prebid_meeting), 'dd/MM/yyyy HH:mm') : 'Not specified'}</div>
<div class="field"><span class="field-label">Pre-bid Query Submission Date:</span> ${formData.prebid_query_submission_date ? format(new Date(formData.prebid_query_submission_date), 'dd/MM/yyyy') : 'Not specified'}</div>
...
<div class="field"><span class="field-label">Tender Fee:</span> ${formData.tender_fee ? `₹${Number(formData.tender_fee).toLocaleString()}` : 'Not specified'}</div>
<div class="field"><span class="field-label">Tender EMD:</span> ${formData.tender_emd ? `₹${Number(formData.tender_emd).toLocaleString()}` : 'Not specified'}</div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            {sharedDocuments.length > 0 && (
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-slate-700">Select an existing document for analysis:</Label>
                  <div className="border rounded-lg p-2 max-h-48 overflow-y-auto space-y-1">
                    {sharedDocuments.map((doc) => (
                      <div
                        key={doc.id}
                        className={`flex items-center justify-between p-2 rounded-md cursor-pointer transition-colors ${selectedDocument === doc.url ? 'bg-blue-100' : 'hover:bg-slate-50'}`}
                      ...
                      >
                        <div className="flex items-center gap-2 overflow-hidden">
                          ...
                          <Badge variant="outline" className="text-xs flex-shrink-0">{doc.uploaded_from}</Badge>
                        </div>
                        <Button
                          type="button"
                          variant="ghost"
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium text-blue-700 mb-2">
                    {isUploading ? "Uploading document..." : "AI analyzing RFP document..."}
                  </p>
                  {isUploading && (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2">
                <Label htmlFor="tender_name" className="text-sm font-medium text-slate-700">Tender Name *</Label>
                <Input
                  id="tender_name"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
               ...
               <div className="space-y-2">
                <Label htmlFor="branches" className="text-sm font-medium text-slate-700">Branches</Label>
                <Input
                  id="branches"
                  type="number"
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <Label htmlFor="submission_date" className="text-sm font-medium text-slate-700">Submission Date</Label>
                <Input
                  id="submission_date"
                  type="date"
              ...
              <div className="space-y-2">
                <Label htmlFor="prebid_meeting" className="text-sm font-medium text-slate-700">Pre-bid Meeting</Label>
                <Input
                  id="prebid_meeting"
                  type="datetime-local"
              ...
              <div className="space-y-2">
                <Label htmlFor="prebid_query_submission_date" className="text-sm font-medium text-slate-700">Pre-bid Query Submission Date</Label>
                <Input
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <Label htmlFor="consultant_name" className="text-sm font-medium text-slate-700">Consultant Name</Label>
                <Input
                  id="consultant_name"
              ...
              <div className="space-y-2">
                <Label htmlFor="consultant_email" className="text-sm font-medium text-slate-700">Email ID Consultant</Label>
                <Input
                  id="consultant_email"
                  type="email"
              ...
              <div className="space-y-2">
                <Label htmlFor="help_desk" className="text-sm font-medium text-slate-700">Help Desk</Label>
                <Input
                  id="help_desk"
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="tender_fee" className="text-sm font-medium text-slate-700">Tender Fee (₹)</Label>
                <Input
                  id="tender_fee"
                  type="number"
              ...
              <div className="space-y-2">
                <Label htmlFor="tender_emd" className="text-sm font-medium text-slate-700">Tender EMD (₹)</Label>
                <Input
                  id="tender_emd"
                  type="number"
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                <Label htmlFor="cbs_software" className="text-sm font-medium text-slate-700">CBS Software</Label>
                <Textarea
                  id="cbs_software"
              ...
              <div className="space-y-2">
                <Label htmlFor="dc" className="text-sm font-medium text-slate-700">DC (Data Center)</Label>
                <Textarea
                  id="dc"
                  value={formData.dc}
              ...
              <div className="space-y-2">
                <Label htmlFor="dr" className="text-sm font-medium text-slate-700">DR (Disaster Recovery)</Label>
                <Textarea
                  id="dr"
                  value={formData.dr}
...
export default function SynopsisPage() {
  ...
  const loadSynopses = useCallback(async () => {
    try {
      const data = await Synopsis.list("-created_date", 50);
      setSynopses(data || []);
    } catch (error) {
  ...
  const loadSharedDocuments = useCallback(async () => {
    try {
      const docs = await UploadedDocument.list("-created_date", 50);
  ...
  const handleRemoveDocument = useCallback(async (docId) => {
    try {
      await UploadedDocument.delete(docId);
  ...
  const handleSubmit = useCallback(async (synopsisData) => {
    try {
      if (editingSynopsis) {
        await Synopsis.update(editingSynopsis.id, synopsisData);
      } else {
        await Synopsis.create(synopsisData);
      }
      setShowForm(true);
      ...
      <div className="h-full p-6">
        <div className="h-full flex flex-col">
          <div className="flex justify-between items-center mb-8">
            ...
            {/* Removed the Create AI Synopsis button */}
          </div>
